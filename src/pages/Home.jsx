...
const Home = ({ auth, name }) => {
  if (!auth.uid) navigate('/login');

  return (
    <Container>
      <h1>{`Cześć ${name}!`}</h1>
      <p>Uprzejmie Donoszę jest ogólnopolskim rozwiązaniem, pomagającym mieszkańcom w nierównej walce z recydywistami parkowania. Wystarczy zrobić dwa zdjęcia, sprawdzić automatycznie pobrany adres i numer rejestracyjny oraz określić rodzaj naruszenia. Tylko tyle. Straż Miejska nie musi odwiedzać tego miejsca żeby stwierdzić to naruszenie naocznie. Dowód ze zdjęcia wraz twoim oświadczeniem jest wystarczający.</p>
      
      <RoundedLink to='/report'>
